...
The learning objectives of this hands-on exercise are as followed:
...
mpsz = st_read(dsn = "Data/geospatial/MasterPlan2014SubzoneBoundaryWebSHP",
               layer = "MP14_SUBZONE_WEB_PL") # all files are binary 
```
...
# st_crs(cyclingpath)  currently in code 9001 (need transform)
```
...
``` r
# Transform the data when we import it
preschool = st_read(dsn = "Data/geospatial/PreSchoolsLocation.kml") %>%
...
::: callout-note
## st_set_crs() vs st_transfrom()

`st_set_crs()`

**Purpose: Assign a CRS to spatial data without transforming it**

-   It sets or changes the CRS label of an `sf` or `sfc` object without modifying the coordinates.
-   Use this when your spatial data already has a known CRS, but it's missing or incorrectly labeled.

`st_transform()`

**Purpose: Reproject spatial data to a new CRS**

-   It **actually transforms the coordinates** from the current CRS to a new one.
-   Use this when you want to change the coordinate system for analysis, visualization, or alignment with other data.
:::
...
listings_sf <- st_as_sf(listings,
                        coords = c("longitude","latitude"), #coords = c(x,y)
                        crs=4326 ) %>% #wgs84
