---
title: "Hands-on 6: Geographical Segmentation with Spatially Constrained Clustering Techniques"
...
```{r}
pacman::p_load(spdep, tmap, sf, ClustGeo, knitr,
...
```{r}
numeric <- ict %>% select_if(is.numeric)
...
Some townships, like Taunggyi and Lashio, have more households/population, resulting in left-skewed distributions.

### 3.3.3 Derive new variables using dplyr package
...
```{r}
selected <- ict_derived %>% select(c(12:17))
...
ggplot(ict_long, aes(x = value)) +
  geom_histogram(bins = 30, fill = "#93A4CC", color = "grey30") +
  facet_wrap(~ variable, scales = "free") +
  labs(title = "Histograms of All Derived Variables",
       x = "", y = "Frequency") +
  theme_minimal()+
  theme(panel.background = element_rect(fill = "#f6f6f6"),
        plot.background = element_rect(fill = "#f6f6f6",color = NA),
...
After the calculating proportions, the distributions become more normally distributed.
...
Histograms are useful for identifying the overall distribution of data values, such as left skew, right skew, or a normal distribution. Box plots, on the other hand, are useful for detecting outliers:
...
radio_bp <- ggplot(data=ict_derived,
                   aes(x=RADIO))+
  geom_boxplot(color="grey30",
               fill="#93A4CC")+
  labs(title = "Boxplot of Radio",
       x = "", y = "Frequency") +  
  theme_minimal()+
  theme(panel.background = element_rect(fill = "#f6f6f6"),
        plot.background = element_rect(fill = "#f6f6f6",color = NA),
...
radio_pr_hist <- ggplot(data=ict_derived, 
                     aes(x=RADIO_PR)) +
  geom_histogram(bins=20, 
                 color="grey30", 
                 fill="#93A4CC")+
  labs(title = "Histogram of Radio_PR",
       x = "", y = "Frequency") +
  theme_minimal()+
  theme(panel.background = element_rect(fill = "#f6f6f6"),
        plot.background = element_rect(fill = "#f6f6f6",color = NA),
...
radio_pr_bp <- ggplot(data=ict_derived,
                   aes(x=RADIO_PR))+
  geom_boxplot(color="grey30",
               fill="#93A4CC")+
  labs(title = "Boxplot of Radio_PR",
       x = "", y = "Frequency") +  
  theme_minimal()+
  theme(panel.background = element_rect(fill = "#f6f6f6"),
        plot.background = element_rect(fill = "#f6f6f6",color = NA),
...
(radio_hist + radio_bp)/(radio_pr_hist + radio_pr_bp)+
  plot_annotation(theme = theme(
    plot.background = element_rect(color= '#f6f6f6', size = 0, fill ="#f6f6f6"))) #change line color and background color
```
...
### 4.2.2 Prepare a choropleth map
...
The Radio_PR provides a more accurate comparative measure of the proportion of households in each township that own a radio.
...
The hierarchical cluster analysis consists of four major steps:
...
cluster_vars <- shan_sf %>%
  ...
  select("TS.x", "RADIO_PR", "TV_PR", "LLPHONE_PR", "MPHONE_PR", "COMPUTER_PR") #internet is dropped
...
Notice that the row number has been replaced into the township name. Now, we will delete the TS.x field by using the code chunk below.
...
Notice the mean and standard deviation of the Z-score standardised clustering variables are 0 and 1 respectively.**Note:** [*describe()*](https://www.rdocumentation.org/packages/Hmisc/versions/4.4-0/topics/describe) of [**psych**](https://cran.r-project.org/web/packages/psych/index.html) package is used here instead of *summary()* of Base R because the earlier provides standard deviation.

::: callout-important
*Z-score standardisation method should only be used if we would assume all variables come from some normal distribution.*
:::

## 6.5 Visualise the Standardised Clustering Variables
...
::: callout-note
The three histograms/density plots confirm that standardisation methods (Min–Max and Z-score) do not change the distributional shape, only the scale and centering of the values
:::

## 6.6 Compute Proximity Matrix
...
## 6.7 Compute Hierarchical Clustering
...
## 6.8 Select the Optimal Clustering Algorithm
...
## 6.9 Determine Optimal Clusters
...
::: callout-note
With reference to the gap statistic graph above, the recommended number of cluster to retain is 1. However, it is not logical to retain only one cluster. By examine the gap statistic graph, the **6-cluster** gives the second largest gap statistic and should be the next best cluster to pick.
:::
...
### 6.11.1 Transform the data frame into a matrix
...
### 6.11.2 Plot interactive cluster heatmap using *heatmaply()*
...
heatmaply(normalize(shan_ict_mat),
  ...
  layout(plot_bgcolor = "#f6f6f6",paper_bgcolor = "#f6f6f6",
         legend = list(
      bgcolor = "#f6f6f6"  # legend background
...
Next, tmap is used to plot the choropleth map showing the cluster formed.

```{r}
hclust.map <- tm_shape(shan_sf_cluster) +
  tm_fill("CLUSTER", 
          palette = "Set2",
          title = "Cluster") +
...
In this section, we will learn how to derive spatially constrained cluster by using [*skater()*](https://r-spatial.github.io/spdep/reference/skater.html)method of [**spdep**](https://r-spatial.github.io/spdep/index.html) package.

## 7.1 Convert into Spatial Polygons DataFrame
...
```{r}
# retrieve coordinates of centroids
...
An MST connects all your townships with the minimum total distance, creating a tree structure with no loops. It's like finding the most efficient way to connect all locations with the fewest/shortest links. In addition, it also helps identify dense clusters (short links between townships), Outliers (long links to isolated townships) and Natural breaks between groups.

MST is often used **BEFORE** hierarchical clustering because:

-   It removes redundant connections
-   Highlights the most important spatial relationships
-   Makes cluster boundaries more interpretable
...
### 7.3.2 Compute minimum spanning tree
...
The means township 38 connects to township 51 with a distance 146.67, and township 51 connects to 30 with a distance 108.37, and etc.
:::
...
## 7.4 Compute spatially constrained clusters using SKATER method

**SKATER** = **Spatial 'K'luster Analysis by Tree Edge Removal**

It's a spatially-constrained clustering method that ensures clusters are **geographically contiguous** (connected).
...
```{r}
clust6 <- spdep::skater(edges = shan.mst[,1:2], # select v1 v2 connections only
                 data = shan_ict, 
                 method = "euclidean", 
                 ncuts = 5) # 6 clusters = 5 cuts
```
...
```{r}
clust6$groups  # clusters each township belongs to
```

```{r}
clust6$edges.groups  # which edges were cut?
...
::: callout-note
It means that 18 townships in cluster 1, 22 townships in cluster 2 and 1 township in cluster 5 and 6.
:::
...
## 7.5 Visualise the clusters in choropleth map
...
shclust.map <-tm_shape(shan_sf_spatialcluster) +
  tm_fill("SP_CLUSTER", 
          palette = "Set2",
          title = "Cluster") +
  tm_borders(alpha = 0.2) +
...
-   α = 0: Only care about ICT similarity (ignore geography) Result: Best ICT clusters, but scattered everywhere
-   α = 1: Only care about geography (ignore ICT) Result: Geographic regions, but mixed ICT levels
-   α = 0.5: Balance both equally Result: Reasonably similar ICT + reasonable contiguity
...
```{r}
clustgeo.map <-tm_shape(shan_sf_ngeo_cluster) +
  tm_fill("CLUSTER", 
          palette = "Set2",
          title = "Cluster") +
  tm_borders(alpha = 0.2) +
...
```{r}
clustgeo_2.map <-tm_shape(shan_sf_Gcluster) +
  tm_fill("CLUSTER", 
          palette = "Set2",
          title = "Cluster") +
  tm_borders(alpha = 0.2) +
...
ggparcoord(data = shan_sf_ngeo_cluster, 
  ...
  theme(plot.background = element_rect(fill = "#f6f6f6",color = NA))
...
-   Kam, T.S. (2025). [Geographical Segmentation with Spatially Constrained Clustering Techniques](https://r4gdsa.netlify.app/chap12.html)
